...
bool Ullmann::check(int d, int k) {
    if (A.label[d] != B.label[k]) return false;
...
bool Ullmann::match(int depth) {
    if (depth >= A.n) return true;
    bool ans = false;
    for (int i = 0; i < B.n; i++) {
...
bool Ullmann::subisomorphism() {
    ...
    for (int i = 0; i < va; i++)
        for (int j = 0; j < vb; j++)
            if (A.deg[i] <= B.deg[j]) M[i][j] = 1;
    F = new int[va];
    memset(F, 0, va * sizeof(int));
    found = match(0);
    delete[] F;
    for (int i = 0; i < va; i++) delete[] M[i];
    delete[] M;
    return found;
}
...
void Ullmann::print_res() {
    if (found) {
        printf("Success.\n");
        for (int i = 0; i < A.n; i++) printf("(%d, %d) ", i, res[i]);
        printf("\n");
    } else
...
void VF2::print_res() {
    if (found) {
        printf("Success.\n");
        for (int i = 0; i < s.G2.n; i++) printf("(%d, %d) ", i, res[i]);
        printf("\n");
    } else
        printf("Failed or haven't run.\n");
}