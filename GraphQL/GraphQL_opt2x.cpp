...
bool qwq_cmp(const qwq &a, const qwq &b) {
  if (a.label == b.label) return a.phisize < b.phisize;
  else return a.label < b.label;
}

bool checkPerfectMatch_label_seperate(vector<int> &a, vector<int> &b, set<int> *Phi, Graph &P) {
  // printf("--- checkPerfectMatch ---\n"); print_vector(a); print_vector(b);
  if (b.size() < a.size()) return false;
  // set<int> set_b;
  // for (int v : b) set_b.insert(v);
  sort(b.begin(), b.end());
  vector<qwq> vecs[40]; // max label 38
  int labelMax = 0;
  for (int u : a) {
    vecs[P.label[u]].push_back(qwq(u, Phi[u].size(), P.label[u]));
    labelMax = max(labelMax, (int)P.label[u]);
  }
  for (int p = 0; p <= labelMax; p++) {
    set<int> cur;
    vector<qwq> &vec = vecs[p];
    sort(vec.begin(), vec.end());
    for (int i = 1; i <= vec.size(); i++) {
      int u = vec[i-1].id;
      int bind = 0; // b index
